...
try:
    ...
    #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    from PF0_Numerisk_Integrasjon import MathCalculations
...
filenameMeas = "måling_numerisk_integrasjon.txt"
...
if not online:    
    ...
    # lister med EGNE VARIABLE i offline-modus. 
    # 
    ...
    sumAreal = [] #blir basisen for y-kolonnen for integrert graf
    nullFlow = 0 #initialverdi beregnes først inne i MathCalculations,
                 #basert på første entry i Lys listen
...
def figureTitles():
    ...
    ax[1,0].set_title('Lys')
    ax[2,0].set_title('Lys')

    # Vær obs på at ALLE delfigurene må inneholde data. 

    ax[0,1].set_xlabel('Tid [sec]')
    ax[1,1].set_title('Lys')
    ax[2,1].set_title('Lys')
...
def plotData():
    ...
    ax[1,0].plot(Tid[0:], Lys[0:], 'b')
    ax[2,0].plot(Tid[0:], Lys[0:], 'b')
    ax[0,1].plot(Tid[0:], Lys[0:], 'b')
    ax[1,1].plot(Tid[0:], Lys[0:], 'b')
    ax[2,1].plot(Tid[0:], Lys[0:], 'b')
...
def offline(filenameMeas, filenameCalcOffline):
    ...
    with open(filenameMeas) as f:
        ...
        for EachRow in MeasurementFromFile:
            ...
            MathCalculations(Tid, Lys, sumAreal, nullFlow)
            print(sumAreal[-1])
            #---------------------------------------------------------
